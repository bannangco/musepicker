...
class TripComAdapter(SourceAdapter):
    source_name = "tripcom"

    def __init__(self, fixture_path: Path | None = None, api_url: str | None = None) -> None:
        super().__init__(fixture_path=fixture_path)
        self.api_url = api_url or os.getenv("TRIPCOM_API_URL")
        self.api_key = os.getenv("TRIPCOM_API_KEY")
    ...
    def fetch_raw_offers(self, since: datetime | None = None) -> AdapterResult:
        ...
        if not isinstance(source_items, list):
            raise ValueError("Trip.com payload must be a list or object with offers list")

        for index, item in enumerate(source_items):
            if not isinstance(item, dict):
                continue
            source_offer_id = str(item.get("offer_id") or item.get("id") or f"tripcom-{index}")
    ...
    def _load_payload(self):
        ...
        raise RuntimeError("Trip.com adapter requires either fixture_path or TRIPCOM_API_URL")
